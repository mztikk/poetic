...
use self::{if_eif_jmp_rewriter::IfEifJmpRewriter, inc_dec_merger::IncDecMerger};
...
impl Optimizer {
    pub fn optimize(&mut self) -> Vec<Instruction> {
        let optimizers: Vec<Box<dyn Optimize>> =
            vec![Box::new(IncDecMerger), Box::new(IfEifJmpRewriter)];
